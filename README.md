...
Bequemer: In der Job-UI auf **⚙ KI-Keys** — dort OpenRouter-Token und
...
## Alles per UI (kein SSH nötig)

Die Job-UI hat eine **🖥 System**-Seite (Token-geschützt):
Dienst-Status + Neustart pro Dienst, Job-UI-Token wechseln (mit Auto-Neustart),
Galerie-Passwort neu setzen, Update aus GitHub per Klick (mit Log).
Dafür legt das Install-Script eine enge sudo-Allowlist an
(`/etc/sudoers.d/hyperframes-suite`, nur Neustarts + `update.sh`) —
bewusst für eine LAN-Box, nicht fürs Internet freigeben.
